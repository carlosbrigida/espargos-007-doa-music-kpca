...
def acquire_clutter(csi_dataset, order=2):
    """
    Same implementation used by Luiz.
    """
...
def main():
    cache_dir = PROJECT_DIR / "data" / "cache_luis"
    ...
    print("=" * 80)
    print("GENERATING CLUTTER ESTIMATES FROM LUIS CACHE")
    print("=" * 80)
    ...
    for scenario_name, filename in SCENARIOS.items():
        ...
        for tx_index, current_mac in enumerate(unique_macs):
            ...
            print(
                f"  TX {tx_index} | "
                f"snapshots={tx_csi.shape[0]}"
            )

            clutter = acquire_clutter(
                tx_csi,
                order=CRAP_ORDER,
            )
