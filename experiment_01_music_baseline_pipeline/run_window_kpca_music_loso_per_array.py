...
def train_apply_kpca_per_array(
    R_train_by_array: dict[int, np.ndarray],
    R_test_all: np.ndarray,
    n_components: int,
    gamma: float,
...
def main() -> None:
    ...
    print()
    print("=" * 80)
    print("LOSO PCA/KPCA ON R + MUSIC - PER ARRAY TRAINING")
    print("=" * 80)
    print("CRAP: disabled")
    print(f"Window size: {window_size}")
    print(f"Stride: {stride}")
    print("Training strategy: one PCA/KPCA model per array")

    all_results = {
        "experiment": "window_trained_r_space_pca_kpca_music_loso_per_array_without_crap",
        "crap_enabled": False,
    ...
    for test_scenario in SCENARIOS:
        ...
        for array_index in range(4):
            print(
                f"Training samples array {array_index}: "
                f"{R_train_by_array[array_index].shape[0]}"
            )
        ...
        for n_comp in pca_components:
            try:
                R_pca_all, pca_stats = train_apply_pca_per_array(
                ...
                pca_results = evaluate_scenario(
                    scenario_key=test_scenario,
                    R_all=R_pca_all,
                    method_name=f"pca_per_array_nc{n_comp}",
                    frequency_hz=frequency_hz,
                    n_sources=n_sources,
                )

                print_results(
                    f"PCA PER ARRAY ON R | n_components={n_comp}",
                    pca_results,
                )
        ...
        for n_comp in kpca_components:
            for gamma in kpca_gammas:
                try:
                    R_kpca_all, kpca_stats = train_apply_kpca_per_array(
                        R_train_by_array=R_train_by_array,
                        R_test_all=R_test_all,
                        n_components=n_comp,
                        gamma=gamma,
                    ...
                    kpca_results = evaluate_scenario(
                        scenario_key=test_scenario,
                        R_all=R_kpca_all,
                        method_name=f"kpca_per_array_nc{n_comp}_g{gamma}",
                        frequency_hz=frequency_hz,
                        n_sources=n_sources,
                    )

                    print_results(
                        f"KPCA PER ARRAY ON R | "
                        f"n_components={n_comp} | gamma={gamma}",
                        kpca_results,
                    )
    ...
    output_file = (
        ...
        / "outputs"
        / "metrics"
        / "window_trained_r_space_pca_kpca_music_loso_per_array_without_crap.json"
    )
